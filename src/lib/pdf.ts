...
export async function generateReportPdf(opts: PdfOpts) {
  ...
  autoTable(doc, { startY: 41, body: summary, theme: "grid", styles: { fontSize: 8.5, cellPadding: 3, fillColor: [246, 250, 247], textColor: [24, 48, 32] }, tableLineColor: [205, 220, 209], tableLineWidth: 0.15 });
  let y = lastY() + 9;
  ...
  const base = { theme: "striped" as const, styles: { fontSize: 7.5, cellPadding: 2.2 }, headStyles: { fillColor: [10, 87, 42], textColor: 255 }, alternateRowStyles: { fillColor: [247, 250, 248] }, margin: { left: 14, right: 14 } };
  ...
  section("Sales details"); autoTable(doc, { ...base, startY: y, head: [["Date", "Branch", "Product", "Customer", "Qty", "Value", "Profit"]], body: opts.sales.map((sale) => [sale.date, sale.branch, sale.product, sale.customer, String(sale.qty), money(sale.qty * sale.price), money(sale.profit)]) }); y = lastY() + 9;
  section("Purchase details"); autoTable(doc, { ...base, startY: y, head: [["Date", "Branch", "Product", "Supplier", "Qty", "Unit cost", "Transport"]], body: opts.purchases.map((purchase) => [purchase.date, purchase.branch, purchase.product, purchase.supplier, String(purchase.qty), money(purchase.price), money(purchase.transport)]) }); y = lastY() + 9;
  section("Expenses"); autoTable(doc, { ...base, startY: y, head: [["Date", "Branch", "Description", "Amount"]], body: opts.expenses.map((expense) => [expense.date, expense.branch, expense.description, money(expense.amount)]) });
